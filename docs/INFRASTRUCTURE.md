...
**Image:** Built from `src/extractous-sidecar/Dockerfile` (Python 3.13 + `extractous` library + Tesseract OCR).
...
**Image:** Built from `src/reranker-sidecar/Dockerfile` (Python 3.13 + `sentence-transformers` + `torch`).
