...
> Test corpus: 1,545 indexed points, hybrid search enabled
...
- **BM25 on/off has negligible latency impact** at this corpus size — keep BM25 enabled.

## Known Issues

- **RRF weight debug field** — `rrf_config` in `/v1/search?debug=true` previously showed no indication whether values were env-configured or defaults (fixed: `custom: true/false` field added in gateway debug response).
